# setfit 0.7.0
# https://github.com/huggingface/setfit/blob/3acbd73f0753c125ae8749eb599f9f2a3421910b/src/setfit/trainer.py
...
from active_learning_lab.thirdparty.sentence_transformers.losses.CosineSimilarityLoss import WeightedCosineSimilarityLoss
from active_learning_lab.thirdparty.setfit.modeling import sentence_pairs_generation_weighted
...
class SetFitTrainerExtended:
    ...
    def __init__(
        ...
        # <changed>
        sentence_pair_generation_method: str = None
        # </changed>
    ):
        ...
        self.sentence_pair_generation_method = sentence_pair_generation_method
    ...
    def train(
        ...
        """
        # <change:disable setting the seed here, we do that "from the outside" in the active learning experiment>
        ...
        if not self.model.has_differentiable_head or self._freeze:
            ...
            if self.loss_class in [
                ...
                if self.train_weights is not None:
                    raise NotImplementedError(f'weights not supported for selected loss {self.loss_class}')
                ...
                if self.train_weights is not None and self.model.multi_target_strategy is not None:
                    raise NotImplementedError('weights not supported for multi_target_strategy')

                if self.train_weights is None:
                    self.train_weights = np.ones((len(y_train),))
                ...
                for _ in trange(self.num_iterations, desc="Generating Training Pairs", disable=not show_progress_bar):
                    if self.model.multi_target_strategy is not None:
                        ...
                        if self.train_weights is not None:
                            train_examples = sentence_pairs_generation_weighted(
                                np.array(x_train), np.array(y_train), self.train_weights, train_examples
                            )
                            train_loss = WeightedCosineSimilarityLoss(self.model.model_body)
                        else:
