# setfit 0.7.0
# https://github.com/huggingface/setfit/blob/3acbd73f0753c125ae8749eb599f9f2a3421910b/src/setfit/modeling.py
import os
import tempfile
...
from active_learning_lab.thirdparty.sentence_transformers.readers.InputExample import WeightedInputExample
from active_learning_lab.thirdparty.sentence_transformers.SentenceTransformer import SentenceTransformer
...
# <change>
def sentence_pairs_generation_weighted(sentences, labels, weights, pairs):
    # Initialize two empty lists to hold the (sentence, sentence) pairs and
    # labels to indicate if a pair is positive or negative
    ...
    for first_idx in range(len(sentences)):
        ...
        # Prepare a positive pair and update the sentences and labels
        # lists, respectively
        positive_weight = 0.5 * (weights[first_idx] + weights[second_idx])
        pairs.append(WeightedInputExample(texts=[current_sentence, positive_sentence], label=1.0, weight=positive_weight))

        third_idx = np.random.choice(negative_idxs[label_to_idx[label]])
        negative_sentence = sentences[third_idx]
        # Prepare a negative pair of sentences and update our lists
        negative_weight = 0.5 * (weights[first_idx] + weights[third_idx])
        pairs.append(WeightedInputExample(texts=[current_sentence, negative_sentence], label=0.0, weight=negative_weight))
    # Return a 2-tuple of our sentence pairs and labels
    return pairs
# </change>
